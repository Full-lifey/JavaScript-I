...
// STRETCH
let carModelsStretch = inventory.map(obj => {
    return obj.car_model
})
carModelsStretch.sort()
...
// STRETCH
let stretchCarYears = inventory.map(obj => {
    return obj.car_year
})
...
console.log(JSON.stringify(findBMWAndAudi()));

// STRETCH
const stretchBMWAndAudi = inventory.filter(cars => {
    return cars.car_make === 'BMW' || cars.car_make === 'Audi'
})
console.log(stretchBMWAndAudi)
